...
<template>
	<div class="mx-auto max-w-2xl px-4 py-6 md:px-6">
		<div class="mb-6 text-center">
			<h2 class="text-2xl font-bold text-surface-800 dark:text-surface-100">Formulario de Pago</h2>
			<p class="mt-1 text-sm text-surface-400 dark:text-surface-500">Revise los datos antes de confirmar</p>
		</div>

		<div class="form-card-page">
			<h3 class="form-section-title">
				<i-material-symbols-credit-card class="text-primary-500"/>
				Método de pago
			</h3>
			<ValidateFormItem label="Seleccionar método" span="12">
				<Select v-model="paymentmethod" :options="filterPaymentMethods" optionLabel="description" option-value="id" fluid
				        placeholder="Elige un método de pago" size="large" @value-change="(value) => onValueSelectPayment(value)"/>
			</ValidateFormItem>
		...
		<div class="form-card-page">
			<h3 class="form-section-title">
				<i-material-symbols-attach-money-rounded class="text-primary-500"/>
		...
		<div class="form-card-page" v-if="labelRateSelected === 'OTRO MONTO'">
			<h3 class="form-section-title">
				<i-material-symbols-attach-money-rounded class="text-primary-500"/>
				Monto a pagar
			</h3>
			<ValidateFormItem span="12">
				<InputNumber v-model="voucheramount" :min="1" prefix="S/ " fluid size="large" @input="refocus"/>
			</ValidateFormItem>
		</div>

		<div class="form-card-page" v-if="dataForViewPayment.description !== 'EFECTIVO'">
			<h3 class="form-section-title">
				<i-material-symbols-receipt-long-outline-rounded class="text-primary-500"/>
				Voucher de pago
			</h3>
			<ValidateFormItem label="Adjuntar comprobante" span="12">
