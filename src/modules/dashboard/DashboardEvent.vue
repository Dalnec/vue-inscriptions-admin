...
interface DashboardData {
    activity: {
        ...
        is_active: boolean;
        is_ended: boolean;
        settings: { inscription: { send_email: boolean; emails: string[] } } | null;
    };
    inscriptions: {
        total: number;
    ...
    churches_ranking: {
        ...
        total_amount: string;
        confirmed_amount: string;
        by_kind: { kind_id: number; kind: string; count: number }[];
    }[];
    groups: {
        total: number;
...
onMounted(async () => {
    ...
    if (!slug) {
        error.value = "No se encontro el evento";
        loading.value = false;
        return;
    }
        ...
        <template v-else-if="data">
                ...
                <div class="relative px-6 py-5 sm:px-8 sm:py-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="text-white">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span v-if="data.activity.shortname" class="text-xs font-mono bg-white/20 px-2 py-0.5 rounded-full">{{ data.activity.shortname }}</span>
                        </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span v-if="data.activity.settings?.inscription?.send_email" class="px-3 py-1 rounded-full text-xs font-semibold bg-blue-300/30 text-blue-100 flex items-center gap-1">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
                    ...
                    <span class="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">{{ data.inscriptions.by_status.CONFIRMADO || 0 }}</span>
                    <span class="text-xs text-gray-400 dark:text-gray-500">inscripciones</span>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
                    ...
                    <span class="text-2xl font-bold text-amber-600 dark:text-amber-400 mt-1">{{ data.inscriptions.by_status.PENDIENTE || 0 }}</span>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
                    ...
                    <span class="text-2xl font-bold text-gray-900 dark:text-white mt-1">S/ {{ formatCurrency(data.financial.total_expected) }}</span>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
                    ...
                    <span class="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">S/ {{ formatCurrency(data.financial.total_posted) }}</span>
                ...
                <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                            ...
                            <span class="text-sm font-bold text-emerald-600 dark:text-emerald-400">S/ {{ formatCurrency(data.financial.total_confirmed) }}</span>
                        </div>
                        <div class="flex items-center justify-between p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                            <span class="text-xs text-gray-600 dark:text-gray-300">Contabilizado</span>
                            <span class="text-sm font-bold text-sky-600 dark:text-sky-400">S/ {{ formatCurrency(data.financial.total_posted) }}</span>
                        </div>
                        <div class="flex items-center justify-between p-2.5 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                            <span class="text-xs font-medium text-amber-700 dark:text-amber-400">Por cobrar</span>
                            <span class="text-sm font-bold text-amber-700 dark:text-amber-400">S/ {{ formatCurrency(data.financial.pending_amount) }}</span>
                        </div>
                    </div>
                    ...
                    <!-- Payment methods -->
                    <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">Por metodo de pago</h4>
                    <div class="space-y-2">
                        ...
                        <div v-if="!data.financial.by_payment_method.length" class="text-xs text-gray-400 dark:text-gray-500 text-center py-2">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div class="p-2 rounded-lg bg-green-50 dark:bg-green-900/20 text-center">
                            ...
                            <span class="text-sm font-bold text-green-700 dark:text-green-300">S/ {{ formatCurrency(data.financial.cash_balance.total_incomes) }}</span>
                        </div>
                        <div class="p-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-center">
                            ...
                            <span class="text-sm font-bold text-red-700 dark:text-red-300">S/ {{ formatCurrency(data.financial.cash_balance.total_expenses) }}</span>
                        </div>
                        <div class="p-2 rounded-lg bg-cyan-50 dark:bg-cyan-900/20 text-center col-span-2">
                            ...
                            <span class="text-sm font-bold text-cyan-700 dark:text-cyan-300">S/ {{ formatCurrency(data.financial.cash_balance.cash_total) }}</span>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr v-for="(church, idx) in data.churches_ranking" :key="church.church_id ?? idx"
                                    ...
                                    <td class="py-2.5 pr-4 font-medium text-gray-900 dark:text-white truncate max-w-[200px]">{{ church.church_name }}</td>
                                    <td class="py-2.5 px-2 text-right font-semibold text-gray-900 dark:text-white">{{ church.total_inscriptions }}</td>
                                    <td class="py-2.5 px-2 text-right text-emerald-600 dark:text-emerald-400 font-medium">{{ church.confirmed }}</td>
                                    <td class="py-2.5 px-2 text-right text-sky-600 dark:text-sky-400 font-medium">{{ church.attended }}</td>
                                    <td class="py-2.5 px-2 text-right text-gray-700 dark:text-gray-300">S/ {{ formatCurrency(church.total_amount) }}</td>
                                    <td class="py-2.5 pl-2 text-right text-emerald-600 dark:text-emerald-400 font-medium">S/ {{ formatCurrency(church.confirmed_amount) }}</td>
                                </tr>
                                <tr v-if="!data.churches_ranking.length">
                                    <td colspan="7" class="py-6 text-center text-sm text-gray-400 dark:text-gray-500">Sin iglesias registradas</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="space-y-2">
                        <div v-for="tarifa in data.financial.by_tarifa" :key="tarifa.tarifa_id"
                            ...
                            <div>
                                ...
                                <div class="text-gray-400 dark:text-gray-500">S/ {{ formatCurrency(tarifa.price) }} x {{ tarifa.inscription_count }}</div>
                            </div>
            ...
            <div v-if="data.churches_ranking.length" class="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 mb-6">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-wider mb-4 flex items-center gap-2">
