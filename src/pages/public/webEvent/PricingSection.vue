...
const fetchRates = async() => {
	...
	try {
		...
		if (response && response?.status === 200) {
			rates.value = (response.data.results as InterfaceRates[]).filter(dt => dt.selected);
		}
	} catch (e) {
		...
		<div class="max-w-6xl mx-auto">
			...
			<div class="text-center mb-16">
				...
				<h2 class="text-3xl md:text-5xl font-info text-white leading-tight motto-font">
					INSCRIBETE YA!<!-- Tarifas disponibles -->
				</h2>
			...
			<div v-else class="grid gap-8"
				...
				<div v-for="(rate, index) in rates" :key="rate.id ?? index"
					...
					<div class="flex flex-col h-full">
						...
						<!-- CTA BUTTON -->
						<Button v-if="rate.active" @click="handleCTA" label="Inscribirme"
						        class="w-full rounded-xl font-semibold py-3 mt-auto"
						        :class="index === 0 ? 'bg-event-accent text-event-bg-dark hover:bg-event-accent-red' : 'bg-white/10 text-white border border-white/20 hover:bg-white/20'"/>
					</div>
				</div>
			</div>
		</div>
