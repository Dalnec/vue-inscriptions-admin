...
const optionsActions = (data: InscriptionsMembers) => {
	...
	const userEvent = userDataStore?.userData?.user;

	const hasConsoleSession = !!userConsole;

	const canManage = hasConsoleSession ? (userConsole?.is_superuser || userConsole?.is_staff) : (userEvent?.profile_description === "ADMINISTRADOR" || userEvent?.is_superuser);
